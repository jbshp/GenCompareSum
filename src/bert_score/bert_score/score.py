...
__all__ = ["score", "plot_example","get_model_and_tokenizer"]

def get_model_and_tokenizer(model_type,device,use_fast_tokenizer=False,all_layers=False):
    if all_layers:
        num_layers = None
    else:
    ...
    model = get_model(model_type, num_layers=num_layers, all_layers=all_layers)
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    model.to(device)
    return num_layers, model, tokenizer
